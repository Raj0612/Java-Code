...
public class CFI_from_API {

    public static void main(String[] args) {
        CFI_from_API fiAPI = new CFI_from_API();
        fiAPI.predicate();
        fiAPI.supplier();
        fiAPI.consumer();
        fiAPI.function();
    ...
    public void predicate() {
        ...
        System.out.println("Predicate " + pStr.test("Vatican City"));

        // BiPredicate<T, U> is a functional interface i.e. one abstract method: boolean test(T t, U u);
        BiPredicate<String, Integer> checkLength = (str, len) -> str.length() == len;
        System.out.println("Predicate " +checkLength.test("Vatican City", 8));
        System.out.println();

    }

    public void supplier() {
        // Supplier<T> is a functional interface i.e. one abstract method:  T get()
    ...
    public void consumer() {
        // Consumer<T> is a functional interface i.e. one abstract method: void accept(T t)
        Consumer<String> printC = s -> System.out.println(s);
        printC.accept("Consumer: " + "To be or not to be, that is the question");
