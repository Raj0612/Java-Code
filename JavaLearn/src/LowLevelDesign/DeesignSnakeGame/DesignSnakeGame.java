package LowLevelDesign.DeesignSnakeGame;
...
public class DesignSnakeGame {
    ...
    public int process(int x, int y){
       // System.out.println("process food " + Arrays.toString(food) + " food length " + food.length + " index " + index);
       // System.out.println("process x: "+ x + " y: " + y);
      //  System.out.println("process queue size " + queue.size());
       ...
       // System.out.println("process queue " + queue.size() + " len " + len + " index " + index);

        //to test whether it bites itself
        for(int[] p: queue){
            if(p[0]==x&&p[1]==y)
                return -1;
        }
       ...
       // System.out.println("process queue size1 " + queue.size());
        for(int i=0; i<queue.size(); i++){
            // int[] arr = (int[]) queue.get(i);
